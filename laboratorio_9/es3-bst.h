...
class BST 
  {
    private:
	  ...
	  // DEVO AGGIUNGERE QUESTI ATTRIBUTI!
   ...
   */
   ...
   void stampa_risultatiR(link& h, BST<Item, Key> b) {
	   ...
	   Item it = b.search(h->item.key()); // cerco l'item in B che ha la stessa matricola

	   float media = ((float)h->item.get_voto() + (float)it.get_voto()) / 2; // calcolo la media
	   if (media >= 18) {
		   cout << it.get_nome() << ", ecco la media delle due parti: " << media << endl;
	   }
	   else {
		   cout << it.get_nome() << ", ecco la media delle due parti: INS" << endl;
	   }

	   // ora la media dei voti sufficienti per ciascuna prova
	   if (h->item.get_voto() >= 18) {
		   studentiA++;
		   mediaA += h->item.get_voto();
	   }
	...
	public:
	  ...
	  void stampa_risultati(BST<Item, Key>& b) {
		  ...
		  cout << "Media voti parte A: " << (mediaA / studentiA) << endl;
		  cout << "Media voti parte B: " << (mediaB / studentiB) << endl;
	  }
  };
